...
def _announce_match(client, rid: str):
    """
    Announce the result of a completed round to the game channel.
    Uses round_state[rid] to pull hashtags and outcome.
    """
    st = round_state.get(rid)
    if not st:
        return
    ...
    # Grab submitted hashtags (may be empty string)
    sa = (st["subs"].get(a) or "").strip()
    sb = (st["subs"].get(b) or "").strip()

    outcome = st.get("game_outcome") or "no outcome recorded"

    # Nice readable text, but generic enough to work with any outcome label
    text_lines = [
        f"*Trial {st['trial']} result:*",
        f"• <@{a}> hashtag: `{sa or '(no hashtag)'}`",
        f"• <@{b}> hashtag: `{sb or '(no hashtag)'}`",
        f"• Outcome: *{outcome}*",
    ]
...
def _send_pair_ephemerals(client, channel_id, a, b, t, rid):
    ...
    # Send ephemerals to both, with button that triggers @app.action("open_submit_modal")
            ...
            blocks=[
                {"type": "divider"},
                {
                    ...
                    "elements": [{
                        ...
                        "style": "primary",
                        "action_id": "open_submit_modal",  # <-- wired to your @app.action
                        "value": rid,
                    }]
                }],
        )
